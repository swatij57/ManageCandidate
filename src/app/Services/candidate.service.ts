...
export class CandidateService {
  ...
  saveCandidateDetail(cand: Candidate): Observable<Candidate> {
    console.log(cand);
    cand.serialnumber = this.CANDIDATE_DATA.length + 1;

    console.log(JSON.stringify(cand))
    const req = this.http.post<Candidate>("http://localhost:7051/api/CandidateDeatil/SaveCandidate", JSON.stringify(cand));
    req.subscribe();
    return req;
